  ...
  useEffect(() => {
    if(audioUrl && audioRef.current) {
        ...
        audioRef.current.play().then(() => {
            // Initialize AudioContext only after user gesture (play)
            if (!audioContextRef.current) {
                ...
                source.connect(dest); // Connect to stream for recording
  ...
  const handleBulkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setPhotos(prev => {
          ...
          // If more files than slots, add new nodes
          while(fileIndex < files.length) {
              newPhotos.push({
                  id: `new-${Date.now()}-${fileIndex}`,
                  url: URL.createObjectURL(files[fileIndex]),
                  version: 0,
                  isEmpty: false
              });
      ...
      {mode === 'tree' && !isRecording && photos.some(p => p.isEmpty) && (
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 z-40">
            <label className="flex flex-col items-center gap-2 cursor-pointer group hover:scale-105 transition-transform duration-300">
                ...
                <div className="px-4 py-2 bg-black/50 backdrop-blur-md rounded-full border border-white/20 text-white font-bold font-handwriting-cn text-xl shadow-lg">
                    一键填满照片
                </div>
                <input 
                    type="file" 
