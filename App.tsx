...
const PolaroidFocus: React.FC<PolaroidFocusProps> = ({ photo, onClose, onReplace }) => {
    ...
    // Generate texture
    const texture = usePolkaDotTexture('#ffffff', ['#ef4444', '#22c55e'], false); // False = not empty
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    saveSettings('appConfig', { treeColorIndex, treeStyle, treeShape });
  }, [treeColorIndex, treeStyle, treeShape]);
    ...
    if(audioUrl && audioRef.current) {
        ...
        const playAudio = async () => {
            try {
                ...
                // Initialize MediaElementSource ONLY ONCE per lifetime of the app to avoid
                // "MediaElementAudioSourceNode can only be created once" error when changing songs.
                ...
                console.error("Audio playback/recording setup failed", e);
            }
        };
  ...
  const handleGesture = useCallback((gesture: GestureType, x: number) => {
      ...
      if (gesture === 'Victory') {
          const now = Date.now();
          if (now - lastColorChange.current > 500) {
              setTreeColorIndex(prev => (prev + 1) % TREE_COLORS.length);
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // 1. Persist Files asynchronously
      let fileIndex = 0;
      ...
      setPhotos(prev => {
          ...
          for (let i = 0; i < newPhotos.length; i++) {
              if (newPhotos[i].isEmpty && fIdx < files.length) {
                  newPhotos[i] = {
                      ...newPhotos[i],
                      url: URL.createObjectURL(files[fIdx]),
                      isEmpty: false,
                      version: newPhotos[i].version + 1
                  };
                  fIdx++;
              }
          }
  ...
  const handleSingleSlotUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0] && targetPhotoId) {
          ...
          await saveFile(targetPhotoId, file); // Save to DB
  ...
  const handleReplacePhoto = async (id: string, file: File) => {
      await saveFile(id, file); // Save to DB
  ...
  const handleGeneratePostcard = async (name: string) => {
    ...
    ctx.fillText('Merry Christmas', width / 2, height * 0.75);
  ...
  const generateVideo = useCallback(() => {
    ...
    // Resume audio context if it's suspended (common browser policy)
    ...
    // Add audio track if available and context is healthy
    let combinedStream = videoStream;
    if (audioDestRef.current && audioDestRef.current.stream.getAudioTracks().length > 0) {
        // Ensure the graph is actually connected by checking destination's activity (implicitly done via graph existence)
    ...
    // -- UPDATED SEQUENCE: Removed Album Browsing, Ends with Greeting --
    setTimeout(() => setShowGreeting(true), 28000);
    setTimeout(() => recorder.stop(), 33000);
      ...
      <div className={`absolute bottom-8 right-8 z-40 flex flex-col items-end gap-4 transition-opacity duration-500 ${isRecording ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <div className={`flex flex-col gap-3 transition-all duration-300 origin-bottom ${menuOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-90 pointer-events-none'}`}>
              ...
              <label className="flex items-center gap-3 cursor-pointer group justify-end">
                  <span className="text-white text-sm font-bold shadow-black/50 drop-shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur px-2 py-1 rounded-lg pointer-events-none">分享贺卡</span>
                  <button 
              ...
              {/* Bulk Upload */}
              <label className="flex items-center gap-3 cursor-pointer group justify-end">
                  <span className="text-white text-sm font-bold shadow-black/50 drop-shadow-md opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur px-2 py-1 rounded-lg pointer-events-none">一键填满</span>
              ...
              <label className="flex items-center gap-3 cursor-pointer group justify-end">
                  ...
                  <div className={`w-12 h-12 rounded-full backdrop-blur-md border transition-all duration-300 flex items-center justify-center shadow-lg hover:scale-110 active:scale-95 ${audioUrl ? 'bg-green-500/30 border-green-400/50' : 'bg-white/10 hover:bg-white/20 border-white/20'}`}>
                      <span className={`text-xl ${audioUrl ? 'animate-spin-slow' : ''}`}>🎵</span>
