...
const App: React.FC = () => {
  ...
  const handleBulkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setPhotos(prev => {
          // Fill available empty slots first
          const newPhotos = [...prev];
          let fileIndex = 0;
          
          for (let i = 0; i < newPhotos.length; i++) {
              if (newPhotos[i].isEmpty && fileIndex < files.length) {
                  newPhotos[i] = {
                      ...newPhotos[i],
          ...
          // REMOVED: The logic that appended new nodes dynamically.
          // This prevents the tree from growing indefinitely and crashing the browser.
          return newPhotos;
      });
      e.target.value = '';
    }
