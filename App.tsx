...
const App: React.FC = () => {
  ...
  return (
    ...
    >
          ...
          <div className="p-6 space-y-8 min-h-full">
            ...
            {/* Color Section */}
                ...
                <div className="flex gap-4 p-2 bg-white/5 rounded-xl justify-around">
                    {TREE_COLORS.map((color, index) => (
                        <button
                            ...
                            className={`w-12 h-12 rounded-full border-4 transition-all duration-300 shadow-lg ${treeColorIndex === index ? 'border-white scale-110 ring-2 ring-yellow-400/50' : 'border-transparent opacity-70 hover:opacity-100 hover:scale-105'}`}
                            style={{ background: color === 'rainbow' ? 'linear-gradient(45deg, #ff0000, #ff8800, #ffff00, #00ff00, #0000ff, #4b0082, #8f00ff)' : color }}
                        />
                    ))}
                </div>
             </div>
             ...
             <div className="space-y-3 pt-4 border-t border-white/10">
                <label className="text-sm text-pink-300 font-bold tracking-wider uppercase">操作 / Actions</label>
                
                <div className="grid grid-cols-2 gap-3">
                    {/* Bulk Upload */}
                    <label className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-xl cursor-pointer transition-colors border border-white/5 hover:border-white/20">
                        <span className="text-2xl mb-2">📷</span>
                        <span className="text-xs font-bold text-white/80">上传照片</span>
                        <input type="file" multiple accept="image/*" onChange={handleBulkUpload} className="hidden" />
                    </label>

                    {/* Music */}
                    <label className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-xl cursor-pointer transition-colors border border-white/5 hover:border-white/20">
                        <span className={`text-2xl mb-2 ${audioUrl ? 'animate-spin-slow' : ''}`}>🎵</span>
                        <span className="text-xs font-bold text-white/80">背景音乐</span>
                        <input type="file" accept="audio/*, .mp3, .wav, .m4a, .aac" onChange={handleAudioUpload} className="hidden" />
                    </label>

                    {/* Generate Video */}
                    <button 
                        onClick={() => { setMenuOpen(false); generateVideo(); }}
                        className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-xl transition-colors border border-white/5 hover:border-white/20"
                    >
                        <span className="text-2xl mb-2">🎥</span>
                        <span className="text-xs font-bold text-white/80">生成视频</span>
                    </button>
                    ...
                    <button 
                        ...
                        className="flex flex-col items-center justify-center p-4 bg-white/5 hover:bg-white/10 rounded-xl transition-colors border border-white/5 hover:border-white/20"
                    >
                        <span className="text-2xl mb-2">{mode === 'album' ? '🎄' : '🖼️'}</span>
                        <span className="text-xs font-bold text-white/80">{mode === 'album' ? '返回圣诞树' : '浏览相册'}</span>
                    </button>
                </div>
                
                 {/* Share Large Button */}
                 <button 
                    onClick={() => { setMenuOpen(false); setShowShareModal(true); setPostcardUrl(null); }}
                    className="w-full py-4 mt-2 bg-gradient-to-r from-red-600 to-pink-600 rounded-xl font-bold shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-3 text-lg"
                 >
                     <span>🎁</span> 制作贺卡
                 </button>
             </div>
      ...
      <div className={`absolute bottom-8 right-8 z-40 transition-opacity duration-500 ${isRecording ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <button 
              onClick={() => setMenuOpen(true)}
              className="w-16 h-16 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(251,191,36,0.4)] border-2 border-white/20 transition-all duration-300 hover:scale-110 active:scale-95 bg-amber-500 hover:bg-amber-400 group"
          >
              <span className="text-3xl text-white drop-shadow-md group-hover:rotate-90 transition-transform duration-500">⚙️</span>
          </button>
      </div>
      ...
      <div className={`absolute inset-0 flex items-center justify-center pointer-events-none transition-all duration-1000 ${showGreeting ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
        <div className="text-center z-40">
            {/* Updated greeting font to Pinyon Script */}
            <h1 className="font-handwriting-en text-7xl md:text-9xl text-[#fbbf24] animate-pulse drop-shadow-[0_0_30px_rgba(251,191,36,0.5)]">
                {customTitle}
            </h1>
        </div>
      </div>
